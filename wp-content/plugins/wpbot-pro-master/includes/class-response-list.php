...
class Response_list extends WP_List_Table {
	 ...
	 * @return mixed
	 */
	public static function get_responses( $per_page, $page_number ) {
		...
		$sql .= ' where 1';
		$per_page = isset($per_page) ? $per_page : $_GET['strlang-filter']; 
        ...
        foreach( $result  as $i => $v ){
			$v["query"] = '<a href="admin.php?page=simple-text-response&action=edit&query='.$v["id"].'&_wpnonce='.wp_create_nonce().'">' . $v["query"] . '</a>';
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'responses_per_page', 20 );
