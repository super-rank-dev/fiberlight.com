...
if ( ! class_exists( 'IL_Admin_Page_Class') ) :
 ...
 */
  ...
  class IL_Admin_Page_Class {
    ...
    public function __construct($args) {
      ...
      add_action('wp_ajax_wbca_delete_history',array( $this,'wbca_delete_history'));
      //plupload ajax
    ...
    public function Load_page_hooker(){
      ...
      if(isset($_POST['action']) && $_POST['action'] == 'save') {
        ...
        global $wpdb;
        if(isset($_POST['admin_able_to_chat'])){
          if($_POST['admin_able_to_chat'] == 'on'){
            $users = get_users();
            $sql_usercheck = "SELECT id FROM {$wpdb->prefix}wbca_department";
            $departments = $wpdb->get_results($wpdb->prepare($sql_usercheck));
    ...
    */
    ...
    public function checkboxlist() {
      $args['type']  = 'checkboxlist';
      $args['id']    = 'checkboxlist';
      $args['std']   = '';
      $this->SetField($args);
    }
    /**
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_checklist( $field, $meta ) {
    if ( ! is_array( $meta ) )
      $meta = (array) $meta;
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        echo "<input type='checkbox' class='rw-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . "' /> <span class='at-radio-label'>{$value}</span>"."\n";
      }
    $this->show_field_end( $field, $meta );
  }
  /**
  ...
  public function addcheckboxlist($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'checklist','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => 'Checkbox Field','options' => $options,'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function wbca_delete_history(){
    global $wpdb;
		$prefix = $wpdb->prefix;
    $client_ids = ($_POST['delete_history_id']);
    foreach ($client_ids as  $value) {
      $sql = "DELETE FROM {$prefix}wbca_message WHERE (user_sender = $value) OR (user_receiver = $value)";
