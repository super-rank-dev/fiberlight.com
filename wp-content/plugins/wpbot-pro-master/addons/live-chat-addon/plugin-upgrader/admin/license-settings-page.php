 ...
 */
class QCLD_wplivechat_License_Settings_page
{
	...
	function register_license_key_settings(){
		register_setting( 'qcld_wplivechat_license', 'qcld_wplivechat_enter_license_key', array($this, 'qcld_callback_quantum_license_key') );
		register_setting( 'qcld_wplivechat_license', 'qcld_wplivechat_enter_envato_key', array($this, 'qcld_callback_envato_license_key') );
		register_setting( 'qcld_wplivechat_license', 'qcld_wplivechat_buy_from_where', array( 'default' => '' ) );
		register_setting( 'qcld_wplivechat_license', 'qcld_wplivechat_site_type', array( 'default' => '' ) );
		//start new-update-for-codecanyon
		register_setting( 'qcld_wplivechat_license', 'qcld_wplivechat_enter_license_or_purchase_key', array( 'default' => '' ) );
		//end new-update-for-codecanyon
	}

	function qcld_callback_quantum_license_key($posted_option){
		if(isset($_POST['submit'])){
			$license_key = sanitize_text_field($_POST['qcld_wplivechat_enter_license_key']);
			$buy_from = sanitize_text_field($_POST['qcld_wplivechat_buy_from_where']);
			$server_type = sanitize_text_field($_POST['qcld_wplivechat_site_type']);
			$plugin_name = wplivechat_LICENSING_PLUGIN_NAME;

			$request = wp_remote_get(
				wplivechat_LICENSING_PRODUCT_DEV_URL."wp-json/qcld-sumu-subscription-checker/v1/checkplugin/?license_key=".$license_key."&plugin_to_check=".$plugin_name
			);
				...
				if ( !is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200 ) {
					...
					if( $result){
						...
						if($requested_plugin_name == $plugin_name){
							...
							delete_wplivechat_invalid_license();
							set_wplivechat_valid_license();
							$track_domain_request = wp_remote_get(wplivechat_LICENSING_PRODUCT_DEV_URL."wp-json/qc-domain-tracker/v1/insertdomain/?license_key=".$license_key."&plugin_name=".$plugin_name."&current_domain=".$current_domain."&plugin_max_domain=".$plugin_max_domain."&buy_from=".$buy_from.'&server_type='.$server_type);
	...
	function qcld_callback_envato_license_key($posted_option){
		if(isset($_POST['submit'])){
			$purchase_code = sanitize_text_field($_POST['qcld_wplivechat_enter_envato_key']);
			$buy_from = sanitize_text_field($_POST['qcld_wplivechat_buy_from_where']);
			$server_type = sanitize_text_field($_POST['qcld_wplivechat_site_type']);
			$plugin_name = wplivechat_LICENSING_PLUGIN_NAME;
			$current_domain_url =  site_url() ;
