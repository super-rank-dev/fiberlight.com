...
class Qcformbuilder_Forms_Admin {
	...
	private function __construct() {
		...
		/**
		 * Runs after Chatbot Form Builder admin is initialized
		 *
		 * @since 1.3.5.3
		 */
	...
	public static function internal_form_templates(){
		...
		$internal_templates = array(
			...
			'simple-contact-example' => array(
				'name'     => esc_html__( 'Simple Contact', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/simple-contact-example.php'
			),
			...
			'bmr-calculator-example' => array(
				'name'     => esc_html__( 'BMR Calculator', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/bmr-calculator-example.php'
			),
			'request-apointment-example' => array(
				'name'     => esc_html__( 'Request Apointment', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/request-apointment-example.php'
			),
			...
			'room-area-calculator' => array(
				'name'     => esc_html__( 'Room Area Calculator', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/room-area-calculator.php'
			),
			'consulting-agency' => array(
				'name'     => esc_html__( 'Consulting Agency', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/consulting-agency.php'
			),
			'event-management' => array(
				'name'     => esc_html__( 'Event Management', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/event-management.php'
			),
			'healthcare-appointment' => array(
				'name'     => esc_html__( 'Healthcare Appointment', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/healthcare-appointment.php'
			),
			'hr-recruiter' => array(
				'name'     => esc_html__( 'HR Recruiter', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/hr-recruiter.php'
			),
			'lead-generation-google' => array(
				'name'     => esc_html__( 'Lead Generation Google', 'qcformbuilder-forms' ),
				'template' => include WFBCORE_PATH . 'includes/templates/lead-generation-google.php'
			),
	 ...
	 * @since unknown
	 */
	public static function qc_browse_entries(){
	...
	public function register_admin_page(){
		...
		if( current_user_can( Qcformbuilder_Forms::get_manage_cap() ) ){
			...
			//add_submenu_page( $this->plugin_slug, 'licensing', 'licensing', 'manage_options','cfb_licensing', array($this, 'qcld_wb_chatbot_admin_page') );
	...
	public function qcld_wb_chatbot_admin_page(){
		?>
			<div id="licensing">
				...
				<?php if( get_convformbuilder_valid_license() ){ ?>
                ...
                $track_domain_request = wp_remote_get(convformbuilder_LICENSING_PRODUCT_DEV_URL."wp-json/qc-domain-tracker/v1/getdomain/?license_key=".get_convformbuilder_licensing_key(), array('timeout' => 300));
                    ...
                    if( !empty($track_domain_result) ){
                        ...
                        //if( $max_domain_num < $total_active_domain ){
                            ...
                            if( !in_array($current_domain, $active_domain_lists) ){
                                set_convformbuilder_invalid_license();
                                delete_convformbuilder_valid_license();
                    ?>
				...
				<form onsubmit="return false" id="qc-license-form" method="post" action="options.php">
					<?php
						...
						settings_fields( 'qcld_convformbuilder_license' );
						do_settings_sections( 'qcld_convformbuilder_license' );

						// if( isset($_POST['submit']) ){
						// 	echo 'qcld_convformbuilder_buy_from_where '.$_POST['qcld_convformbuilder_buy_from_where'];
						// }
					?>
					<table class="form-table">
						...
						<tr id="quantumcloud_portfolio_license_row" style="display: none">
							<th>
								<label for="qcld_convformbuilder_enter_license_key">Enter License Key:</label>
							</th>
							<td>
								<input type="<?php echo (get_convformbuilder_licensing_key()!=''?'password':'text'); ?>" id="qcld_convformbuilder_enter_license_key" name="qcld_convformbuilder_enter_license_key" class="regular-text" value="<?php echo get_convformbuilder_licensing_key(); ?>">
						...
						<tr id="show_envato_plugin_downloader" style="display: none">
							<th>
								<label for="qcld_convformbuilder_enter_envato_key">Enter Purchase Code:</label>
							</th>
							<td colspan="4">
								<input type="<?php echo (get_convformbuilder_envato_key()!=''?'password':'text'); ?>" id="qcld_convformbuilder_enter_envato_key" name="qcld_convformbuilder_enter_envato_key" class="regular-text" value="<?php echo get_convformbuilder_envato_key(); ?>">
						...
						<tr>
							<th>
								<label for="qcld_convformbuilder_enter_license_or_purchase_key">Enter License Key or Purchase Code:</label>
							</th>
							<td>
								<input type="<?php echo (get_convformbuilder_license_purchase_code()!=''?'password':'text'); ?>" id="qcld_convformbuilder_enter_license_or_purchase_key" name="qcld_convformbuilder_enter_license_or_purchase_key" class="regular-text" value="<?php echo get_convformbuilder_license_purchase_code(); ?>" required>
							</td>
						</tr>
					...
					<input type="hidden" name="qcld_convformbuilder_buy_from_where" value="<?php echo get_convformbuilder_licensing_buy_from(); ?>" >
				...
				<script type="text/javascript">
					jQuery(document).ready(function(){
						...
						//start new-update-for-codecanyon
						jQuery('#qcld_convformbuilder_enter_license_or_purchase_key').on('focusout', function(){
							qc_convformbuilder_set_plugin_license_fields();
						});

						jQuery('#qcld_convformbuilder_enter_license_or_purchase_key').on('keypress',function (e) {
							  qc_convformbuilder_set_plugin_license_fields();
						});
						...
						function qc_convformbuilder_set_plugin_license_fields(){
							var license_input = jQuery('#qcld_convformbuilder_enter_license_or_purchase_key').val();
							if( /^(\w{8})-((\w{4})-){3}(\w{12})$/.test(license_input) ){
								jQuery('input[name="qcld_convformbuilder_buy_from_where"]').val('codecanyon');
								jQuery('input[name="qcld_convformbuilder_enter_envato_key"]').val(license_input);
							}else{
								jQuery('input[name="qcld_convformbuilder_buy_from_where"]').val('quantumcloud');
								jQuery('input[name="qcld_convformbuilder_enter_license_key"]').val(license_input);
							}
						}
						//end new-update-for-codecanyon
	...
	public function enqueue_admin_stylescripts() {
		...
		/**
		 * Control if Chatbot Form Builder assets run in post editor
         *
         * @since 1.5.7
         *
		...
		if ( $screen->base === 'post' && apply_filters( 'qcformbuilder_forms_insert_button_include', true, get_post_type() ) ) {
            /**
             * This action causes the assets Chatbot Form Builder loads in the post editor to be enqueued
			 *
			 * @since 1.7.3
             */
        ...
        /**
         * This action causes the assets Chatbot Form Builder loads in the main admin page to be enqueued
         *
         * @since 1.7.3
         */
		...
		if ( Qcformbuilder_Forms_Admin::is_edit() ) {
            ...
            /**
             * This action causes the assets Chatbot Form Builder loads in the form editor to be enqueued
             *
             * @since 1.7.3
             */
	...
	/**
	 * Set panels to be used in Chatbot Form Builder form editor
	 *
	 * @since unknown
	 *
			...
			'form_layout' => array(
				...
				"tabs"		=>	array(
					"layout" => array(
						"name" => __( 'Layout', 'qcformbuilder-forms' ),
						"location" => "lower",
						"label" => __( 'Layout Builder', 'qcformbuilder-forms' ),
						"active" => true,
						"actions" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/doc/form-layout-grid-builder/?utm_source=wp-admin&utm_medium=form-editor&utm_term=email-tab',
							'text' => __( 'Chatbot Form Builder layout builder getting started guide', 'qcformbuilder-forms' ),
						)
					),
					/*
					"pages" => array(
					...
					*/
					"mailer" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/qcformbuilder-forms-emails/?utm_source=wp-admin&utm_medium=form-editor&utm_term=tabs',
							'text' => __( 'Learn to use the Chatbot Form Builder email notification', 'qcformbuilder-forms' )
						)
					),
					"processors" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/doc/using-form-processors/?utm_source=wp-admin&utm_medium=form-editor&utm_term=tabs',
							'text' => __( 'Processors getting started guide', 'qcformbuilder-forms' )
						)
					),
					...
					"conditions" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/doc/using-form-conditionals/?utm_source=wp-admin&utm_medium=form-editor&utm_term=tabs',
							'text' => __( 'Conditionals getting started guide', 'qcformbuilder-forms' )
						)
					),
					"variables" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/doc/using-form-variables/?utm_source=wp-admin&utm_medium=form-editor&utm_term=tabs',
							'text' => __( 'Variables getting started guide', 'qcformbuilder-forms' )
						)
					),
					"revisions" => array(
						...
						'tip' => array(
							'link' => 'https://quantumcloud.com/doc/form-revisions-drafts/?utm_source=wp-admin&utm_medium=form-editor&utm_term=tabs',
							'text' => __( 'Working with form revisions and drafts', 'qcformbuilder-forms' )
						)
					),
					/*
					"
    ...
    /**
     * Remove hooks for admin notices while in Chatbot Form Builder admin
     *
     * Chatbot Form Builder admin does not play nice with admin notices, so we use a series of steps to remove most of them, sadly can not beat them all.
     *
     * @since 1.4.7
	...
	/**
	 * Check if is a Chatbot Form Builder page
	 *
	 * @since 1.5.0.9
	 *
    ...
    protected static function core_privacy_content() {
        return
            '<h2>' . esc_html__( 'Chatbot Form Builder: Data Collection', 'qcformbuilder-forms') . '</h2>' .
            '<em>' . esc_html__( 'Suggested Text (if you have entry tracking enabled) -', 'qcformbuilder-forms' ) . '</em>' .
            '<p>' . esc_html__( 'Chatbot Form Builder stores a record of all form submissions. Your data may be deleted by the site administrator. You may request a report of saved data related to your email address.', 'qcformbuilder-forms' ) . '</p>' .
            '<em>' . esc_html__( 'Suggested Text (if you do not have entry tracking enabled) -', 'qcformbuilder-forms' ) . '</em>' .
            '<p>' . esc_html__( 'Chatbot Form Builder stores data only briefly for each submission. Uploaded media files may remain on the server' ) . '</p>' .
    ...
    /**
     * Return the default suggested privacy policy content for Chatbot Form Builder Pro.
     *
     * @since 1.7.0
     *
     * @return string The default policy content.
    ...
    protected static function pro_privacy_content() {
        return
            '<h2>' . esc_html__( 'Chatbot Form Builder Pro: Data Collection', 'qcformbuilder-forms') . '</h2>' .
            '<p>' . esc_html__( 'This site uses a third-party service to deliver some emails.', 'qcformbuilder-forms' ) . '</p>' .
            '<p>' . esc_html__( 'This data will be stored on servers controlled by QcformbuilderWP LLC, the makers of Chatbot Form Builder, and operated by Linode and Amazon Web Services. Meta data about emails will be stored by SendGrid and Amazon Web Services', 'qcformbuilder-forms' ) . '</p>';
    ...
    public static function add_suggested_privacy_content() {
        if( function_exists( 'wp_add_privacy_policy_content' ) ){
            wp_add_privacy_policy_content( esc_html__( 'Chatbot Form Builder', 'qcformbuilder-forms' ), self::core_privacy_content() );
            if ( qcformbuilder_forms_pro_is_active() ) {
                wp_add_privacy_policy_content(esc_html__('Chatbot Form Builder Pro', 'qcformbuilder-forms'), self::pro_privacy_content());
            }
        }
    }
