...
if(!empty($element['layout_grid']['fields'])){
	foreach($element['layout_grid']['fields'] as $field_id=>$location){
		...
		if(isset($element['fields'][$field_id])){
			...
			qcformbuilder_forms_field_wrapper_template( $field_id, $field['label'], $field['slug'], @$field['additional'] , @$field['show_as_menu'] , @$field['caption'], ( isset($field['hide_label']) ? 1 : 0 ), ( isset($field['required']) ? 1 : 0 ),( isset($field['entry_list']) ? 1 : 0 ), $field['type'], $config_str, $conditions);
