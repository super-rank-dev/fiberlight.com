<?php
class QCLD_convformbuilder_AutoUpdate 
{
	/**
	...
	{
		$qcld_renew_subscription = get_convformbuilder_renew_transient();
	...
	public function check_update( $transient )
	{
		...
		if ( version_compare( $this->current_version, $remote_version->new_version, '<' ) ) {
			...
			if( $this->slug == $subscription_enable ){
				...
				set_convformbuilder_update_transient($obj);
				delete_convformbuilder_renew_transient();
				
				$transient->response[$this->plugin_slug] = $obj;
			 }else{
			 	$obj = new stdClass();
			 	set_convformbuilder_renew_transient($obj);
			 	$qcld_renew_subscription = get_convformbuilder_renew_transient();
			 	if( $qcld_renew_subscription ){
			 		add_action("after_plugin_row_{$this->plugin_slug}", array($this, 'show_upgrade_subscription_on_plugin_row'), 10, 3 );
			 	}
	  		...
	  		echo '<tr class="plugin-update-tr">
	  			<td colspan="3" class="plugin-update colspanchange">
		        	<div class="update-message notice inline notice-warning notice-alt">
		        		<p>There is a new version available. <a href="'.esc_url('https://www.ultrawebmedia.com/li/plugins/conversational-form-builder/changelog.txt').'" target="_blank">View version details</a>. Automatic update is unavailable for this plugin. To receive automatic updates, valid license is required.</p>
		        	</div>
		        </td>
		    </tr>';
		}
	...
	{
		$qcld_update_plugin = get_convformbuilder_update_transient();
