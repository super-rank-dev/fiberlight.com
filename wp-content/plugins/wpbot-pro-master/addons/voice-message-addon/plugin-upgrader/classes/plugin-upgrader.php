<?php
class QCLD_wpbotvoicemessage_AutoUpdate 
{
	/**
	...
	public function check_update( $transient )
	{
		...
		if ( version_compare( $this->current_version, $remote_version->new_version, '<' ) ) {
			...
			if( $this->slug == $subscription_enable ){
				...
				set_wpbotvoicemessage_update_transient($obj);
				delete_wpbotvoicemessage_renew_transient();
				
				$transient->response[$this->plugin_slug] = $obj;
			 }else{
			 	$obj = new stdClass();
			 	set_wpbotvoicemessage_renew_transient($obj);
			 	$qcld_renew_subscription = get_wpbotvoicemessage_renew_transient();
			 	if( $qcld_renew_subscription ){
			 		add_action("after_plugin_row_{$this->plugin_slug}", array($this, 'show_upgrade_subscription_on_plugin_row'), 10, 3 );
			 	}
	...
	public function show_upgrade_subscription_on_plugin_row( $plugin_file, $plugin_data, $status ) {
		...
		if( $qcld_renew_subscription && get_wpbotvoicemessage_license_purchase_code() ){
	  		echo '<tr class="plugin-update-tr wpbotvoicemessage-invalid-license">
	  			<td colspan="3" class="plugin-update colspanchange">
		        	<div class="update-message notice inline notice-warning notice-alt">
		        		<p>There is a new version available. <a href="'.admin_url('plugin-install.php?tab=plugin-information&plugin='.wpbotvoicemessage_LICENSING_PLUGIN_NAME.'&section=changelog&TB_iframe=true&width=772&height=520').'" class="thickbox open-plugin-details-modal" >View version details</a>. Automatic update is unavailable for this plugin. To receive automatic updates, valid license is required. Updates are crucial for compatibility and security.</p>
		        	</div>
		        </td>
		    </tr>';
		}
	...
	{
		$qcld_update_plugin = get_wpbotvoicemessage_update_transient();
