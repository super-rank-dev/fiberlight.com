...
class TablePress_Modules_View extends TablePress_View {
	 ...
	 */
	public function textbox_modules( array $data, array $box ) {
		foreach ( $data['categories'] as $category => $category_name ) {
			...
			foreach ( $data['available_modules'][ $category ] as $slug => $module ) {
				...
				echo '<a href="' . esc_url( "https://tablepress.org/modules/{$slug}/" ) . '" class="module-link">' . __( 'More Details', 'tablepress' ) . '</a>';
				...
				echo '</label>';
				if ( '' !== $disabled ) {
					echo '<div class="ribbon"><span>' . sprintf( '<a href="%1$s">%2$s</a>', 'https://tablepress.org/premium/', sprintf( __( '%s plan', 'tablepress' ), $plan ) ) . '</span></div>';
				}
				echo '</div>';
			}
			echo '</div>';
