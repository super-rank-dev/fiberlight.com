...
class TablePress_Module_Automatic_Table_Export {
	...
	/**
	 * Automatic Table Export configuration.
	 *
	 * @var array
	 * @since 2.0.0
	 */
	protected static $auto_export_config = array();
	 ...
	 * @since 2.0.0
	 */
	public static function init_automatic_table_export() {
	 ...
	 */
	public static function perform_automatic_export( $table_id ) {
	 ...
	 */
	public static function change_export_view_full_path( $full_path, $file, $folder ) {
	 ...
	 */
	public static function change_view_export_class_name( $class_name ) {
	 ...
	 */
	public static function add_automatic_table_export_view_data( array $data, $action ) {
		if ( 'export' !== $action ) {
			return $data;
		}
	 ...
	 * @since 2.0.0
	 */
	public static function handle_ajax_action_automatic_table_export() {
		...
		$auto_export_config = (array) json_decode( $auto_export_config, true );
		...
		if ( ! isset( $auto_export_config['active'], $auto_export_config['path'], $auto_export_config['formats'] ) || ! is_array( $auto_export_config['formats'] ) ) {
			wp_die( '-1' );
		}
		...
		$configuration = array(
			...
			'formats'       => array(),
			'csv_delimiter' => $auto_export_config['csv_delimiter'],
		);

		$allowed_formats = array( 'csv', 'json', 'html' );
		foreach ( $auto_export_config['formats'] as $format ) {
