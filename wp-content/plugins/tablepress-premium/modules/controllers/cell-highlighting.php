...
class TablePress_Module_Cell_Highlighting {
	 ...
	 */
	public static function postbox_cell_highlighting( array $data, array $box ) {
		$help_box_content = '';
	 ...
	 */
	public static function _cell_partial_match( $a, $b ) {
	 ...
	 * @param array $orig_table     The previous state of the table, including hidden rows/columns.
	...
	public static function process_parameters( $table, $orig_table, $render_options ) {
		...
		if ( self::$full_cell_match ) {
			...
			self::$highlight_match_function = '_cell_partial_match';
			if ( $render_options['highlight_case_sensitive'] ) {
	 ...
	 */
	public static function highlight_cells( $cell_class, $table_id, $cell_content, $row_number, $column_number, $colspan, $rowspan ) {
		if ( '' === $cell_content ) {
			return $cell_class;
		}
		...
		foreach ( self::$highlight_terms as $highlight_term ) {
			if ( call_user_func( array( __CLASS__, self::$highlight_match_function ), $cell_content, $highlight_term ) ) {
	 ...
	 */
	public static function remove_cell_css_class_filter( $output, $table, $render_options ) {
		remove_filter( 'tablepress_cell_css_class', array( __CLASS__, 'highlight_cells' ), 10, 7 );
		return $output;
	}
