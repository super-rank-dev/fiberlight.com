...
abstract class TablePress_Controller {
	...
	protected function plugin_update_check() {
		...
		if ( $current_plugin_options_db_version < TablePress::db_version ) {
			...
			// Trigger update of the table options as premium modules might have new table options, when updating from a version older than TablePress 2.0.4.
			if ( $current_plugin_options_db_version < 53 ) {
